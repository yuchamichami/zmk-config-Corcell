...
| ファイル | 用途 |
| --- | --- |
| `Corcell_R-dya-studio-xiao_ble_zmk.uf2` | 出荷用・右側（PAW3222、DYA Studio対応） |
| `Corcell_L-dya-studio-xiao_ble_zmk.uf2` | 出荷用・左側（PAW3222） |
| `settings_reset-dya-studio-xiao_ble_zmk.uf2` | 復旧用の設定消去。通常の書込み・更新には使いません |

**2026-09-26の整理後のビルド**を使用してください。古いActionsのZIPにはテスト版が含まれている場合があります。
30秒スリープ、センサー停止、エンコーダー停止、センサー処理比較、IQS9151 A2評価の各ファームは、このブランチから取り除きました。
通常版はログ・シェルOFF、PAWとエンコーダー有効、既定のdeep sleepは15分です。Studioで保存した設定がある場合はそちらも確認してください。
...
- 配布物を左右のDYA Studio通常版と復旧用settings_resetの3ファイルへ整理しました。テスト専用のビルド設定・コード・IQS9151依存を削除しました。
