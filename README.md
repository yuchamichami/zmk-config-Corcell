...
独自TPS43-outline A2の通常評価版・USB診断版を追加しています。
RESET未接続、FR4基板1mm・オーバーレイ1mmの未調整プロファイルです。
書込み前に[配線・ログ・実機テスト手順](docs/iqs9151-a2.md)を確認してください。
旧NCSのGND固定はそのまま接続できません。キーマップは既存版を維持しています。
